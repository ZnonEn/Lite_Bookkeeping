...
- 确认卡片需要「显示在其他应用上层」权限；跨通道自动去重
- 已知限制：微信发出红包/转账的过程中没有金额信息，无法自动记录

### 自动分类
按优先级依次尝试，命中即用：
1. **商户记忆**：你亲手改过分类的商户，下次自动沿用（改分类即自动积累）
2. **自定义规则**：在规则页写下的关键词 → 分类映射
3. **平台自带分类**：支付宝「交易分类」、微信「交易类型」的权威值
4. **内置规则**：500+ 条预置商户与场景关键词
5. 兜底「其他」
收/支方向各自独立判断；规则页可查看与清空商户记忆。
...
- 无障碍 / 通知使用权仅用于读取支付结果文本或通知，不做任何点击、不联网
...
| 存储 | Room + DataStore |
| 解析 | 自研 xlsx / csv 读取器（微信、支付宝账单与本应用 Excel 备份） |
...
└── com/nonen/Bookkeeping/
    ...
    │   ├── motion/      # 动效（弹簧、按压缩放）
    │   ├── screens/     # 主页 / 统计 / 设置 / 记一笔 / 搜索 / 规则（页面与 ViewModel 分文件）
    │   └── theme/       # 配色与字体（深浅双主题）
    └── export/      # Excel 备份导出
```
...
本项目采用 [Apache License 2.0](https://www.apache.org/licenses/LICENSE-2.0) 开源协议：可自由使用、修改、再分发与商用，须保留版权与协议声明。完整协议文本见 [LICENSE](LICENSE)。
